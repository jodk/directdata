...
import static eql.engine.QueryDataUtils.*;

public class NoNestQueryScreenDefaultImpl implements QueryScreen {
	@Override
				...
				if(multifilters!=null){//
					...
					for(Map.Entry<String, List<Filter>> entry : groupMap.entrySet()){
						...
						if(dbcs !=null && dbcs.length>0) {
							changeColumnsIndex(dbcs, selectedColumnIdxList);
							changeName(dbcs, groupName, groupbys.length);
							queryDataList.add(qd);
						}
					}
//					
					QueryData qd = mergeColumn(queryDataList, groupbys.length);
					//合并后，如果维度列有排序功能，则再进行排序
					sortQueryData(qd, orderbys);
					result =  qd;
				}else{
					System.out.println("build sql : " + expression.getSQL());
					QueryData qd = dbs.query(expression.getSQL());
				...
				if(groupfilters !=null){
					result = mergeRow(result, groupfilters, groupbys.length);
				}
			}
			return result;
		}catch(Exception e){
